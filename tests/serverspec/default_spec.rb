...
extra_localhost = ["localhost1", "localhost2", "buz.example.org"]
...
describe file(hosts_file) do
  ...
  its(:content) { should match(/127\.0\.0\.1\s+localhost\s+#{Regexp.escape(extra_localhost.join(" "))}$/) }
